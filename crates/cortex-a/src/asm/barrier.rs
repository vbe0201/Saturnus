...
barrier_types![
    ...
    /// types, both before and after the barrier instruction.
    ISH: "ish",
    ...
    /// Inner Shareable is the required shareability domain, reads are the required access type before
    /// the barrier instruction, and reads and writes are the required access types after the barrier
    /// instruction.
    ISHLD: "ishld",

    /// Non-shareable is the required shareability domain, reads and writes are the required access,
    ...
    /// Non-shareable is the required shareability domain, reads are the required access type before
    /// the barrier instruction, and reads and writes are the required access types after the barrier
    /// instruction.
    NSHLD: "nshld",
...
/// Data Memory Barrier is a memory barrier that ensures the ordering of observations of memory
/// accesses.
///
...
/// Data Synchronization Barrier is a memory barrier that ensures the completion of memory
/// accesses.
///
/// The type of this memory barrier can be specified using the `T` generic type.
#[inline(always)]
...
/// Instruction Synchronization Barrier flushes the pipeline in the PE and is a context
/// synchronization event.
///
/// The type of this memory barrier is always [`SY`] as this is the only valid option
/// for a `isb` barrier.
#[inline(always)]
pub unsafe fn isb() {
    match () {
