...
//! # Example
//!
//! This example whill show how we terminate the QEMU session. The program should be
//! running under QEMU with semihosting enabled (use `-semihosting` flag).
//!
//! Target program:
//!
//! ```no_run
...
///
/// This call can be used to terminate QEMU session and report back success or failure.
/// If you need to pass more than one type of error, consider using [`report_exception`]
/// syscall instead.
///
/// This call should not return. However, it is possible for the debugger to request
...
///
/// Exception handlers can use this SWI at the end of handler chains as the default
/// action to indicate that the exception has not been handled.
///
/// This call should not return. However, it is possible for the debugger to request
/// that the application continues. In this case, this call returns normally.
